...
class ElementaryNeuronNet:
    ...
    def __init__(self, hidden_layer_neuron_count, output_layer_neurons_count, input_data_shape, lean_rate=0.0001,
                 epochs=50000):
        np.random.seed(1)
        self.epochs = epochs
    ...
    def limit_function(self, x: np.ndarray):
        vectorized_limit = np.vectorize(self.lim_fun)
        return vectorized_limit(x)

    def lim_fun(self, x):
        limit = self.learn_rate
        if x < -limit:
            return -limit
        elif x < limit:
            return limit
    ...
    def train(self, train_data, expected_outcome: list):
        ...
        for epoch in range(0, self.epochs + 1):
            ...
            comb_strength_on_last_neuron = self.activation_function(
                np.dot(comb_strength_on_hidden_neurons, self.weights_final))
            ...
            adjustment_weights_on_last = self.limit_function(np.dot(comb_strength_on_hidden_neurons.T, delta_last))
            ...
            if (0 < epoch < 10) or epoch % 100 == 0 or epoch == self.epochs - 1:
